...
ggplot(twoHT_ROC_points_avg,
       aes(x=FAR, y=HR, color=strength, shape = factor(pOld))) +
  geom_point(size=4) +
  geom_line(aes(linetype=model, group=interaction(strength, model)),
            data = filter(twoHT_ROC_points_avg, model=="obs"),
            size=1.75
            ) +
  geom_line(aes(x=FAR, y=HR, color=strength, linetype=model),
            data = mutate(twoHT_ROC_curve_avg, type="pred"),
            size=1.75,
            inherit.aes = FALSE
            ) +
