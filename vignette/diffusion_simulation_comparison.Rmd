...
theta <- filter(Y, diff == min(abs(diff))) %>%
  slice(1) %>%
...
However, it should be noted that despite large differences in counts, the value of the RT quantiles between these two methods are not very different. For example, using the "worst" set off parameters from earlier, the difference between the RT cutoff value for the first decile of lower boundary responses was `r abs(q_rtdists_0 - q_sim_0)*1000` milliseconds, and the difference between the RT cutoff value for the first decile of upper boundary responses was `r abs(q_rtdists_1 - q_sim_1)*1000` milliseconds
