...
sample_parameters <- function(N=500) {
  ...
  parameters$z <- parameters$z * parameters$a
  return(parameters)
}
...
weights <- as.vector(table(nCells))/length(null_fits)
...
rdiffusion_obj <- function(theta, data, fixed=NULL, Nsim=5e4) {
  ...
  set.seed(42)
  sim_data <- rdiffusion(n=Nsim, a = theta["a"], z = theta['z'],
                         v = theta["v"], sv = theta["sv"],
                         t0 = theta["t0"], st0 = theta["st0"],
                         stop_on_error=FALSE)
...
diffusion_SDT_collapsed_obj <- function(theta, data, fixed=NULL, Nsim=5e4) {
  ...
  theta <- c(theta, fixed)
  boundaries <- c(0,1) # Boundaries to loop over
  set_diffusion_SDT_seed(22)
  sim_data <- diffusion_SDT(Nsim, a = theta["a"], z = theta['z'],
                            v = theta["v"], sv = theta["sv"],
                            t0 = theta["t0"], st0 = theta["st0"]
                            )
