...
write.csv(twoLT_DR_params_scale_subject,
...
for (s in unique(speeded_RT_choice_quantiles$subject)) {
  ...
  subject_params <- filter(twoLT_DR_params_scaled_subject, subject == s)
  kable(filter(twoLT_DR_params_scaled_subject, subject == s),
        digits = 3,
