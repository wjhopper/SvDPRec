...
twoLT <- function(theta) {
  ...
  b <- theta[c("GO_Cons", "GO_Neut", "GO_Lib")]
  O_DN <- abs(pmin(b, 0))
  O_FD <- pmax(pmin(1-b, 1), 0)
  O_DO <-  2 - pmax(b, 1)
  
  FAR <- theta["DO_L"]*O_DO + theta["FD_L"]*O_FD + theta["DN_L"]*O_DN
  HR_W <- theta['DO_W']*O_DO + theta["FD_W"]*O_FD + theta["DN_W"]*O_DN
  HR_S <- theta['DO_S']*O_DO + theta["FD_S"]*O_FD + theta["DN_S"]*O_DN
...
twoLT_plus_DR <- function(theta, data, fixed=NULL) {
  ...
  twoLT_negLL <- twoLT_LL(detect=theta[c("dNew.L", "dOld.W", "dOld.S")],
                          guess=theta[c("gC", "gN", "gL")], #this order matters
                          counts = data$biased)
  
  DR_negLL <- DR_obj(theta[c("dNew.L", "dOld.W", "dOld.S", "gOld",
                            "mu.d", "mu.g", "sigma", "nu")],
                     data = data$speeded)
...
fits <- foreach(sub = subjectwise_datalist,
    ...
    fit <- optimx(theta,
                  ...
                  method = "nlminb",
                  itnmax = 3000,
                  lower = c(-Inf,-Inf, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf, -Inf, 0.01, 0.01, .01, .01),
                  upper = c(Inf,  Inf,  Inf,  Inf,  Inf,  Inf,  Inf,  Inf,  Inf, 2,    2,    .5,  .5),
