...
```{r RT, cache=TRUE, cache.extra=list(diffusion_SDT_R, tools::md5sum(file.path(root_dir, "src", "diffusion.cpp"))), fig.width=8}
A <- as.data.frame(diffusion_SDT(1e6, a=1.5, v=1, t0=.5, z=.6, sz=.25, st0=.25, sv=.5)[,1:2])
names(A) <- c("rt", "response")
B <- rdiffusion(1e6, a=1.5, v=1, t0=.5, z=.6, sz=.25, st0=.25, sv=.5)
B$response <- as.numeric(B$response) - 1
...
```{r RT2, cache=TRUE, cache.extra=list(diffusion_SDT_R, tools::md5sum(file.path(root_dir, "src", "diffusion.cpp"))), fig.width=8}
A <- as.data.frame(diffusion_SDT(1e6, a=1.5, v=1, t0=.5, z=.1, st0=.25, sv=.5)[,1:2])
names(A) <- c("rt", "response")
B <- rdiffusion(1e6, a=1.5, v=1, t0=.5, z=.1, st0=.25, sv=.5)
B$response <- as.numeric(B$response) - 1
