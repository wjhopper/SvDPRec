...
qDR <- function(p, correct, DO, FD, b, mu.do, mu.g, mu.dn, sigma, nu, reverse=FALSE) {
  ...
  N_DO_trials <- round(weights["DO"]*n_samples)
  
  if (N_DO_trials == 0) {
    DO_RTs <- rep(Inf, n_samples)
  } else {
    DO_RTs <- c(gamlss.dist::rexGAUS(N_DO_trials,
                                 mu = mu.do, sigma = sigma, nu = nu
                                 ),
                rep(Inf, n_samples - N_DO_trials)
                )
  }

  N_DN_trials <- round(weights["DN"]*n_samples)
  if (N_DN_trials == 0) {
    DN_RTs <- rep(Inf, n_samples)
  } else {
    DN_RTs <- c(gamlss.dist::rexGAUS(N_DN_trials,
                                     mu = mu.dn, sigma = sigma, nu = nu
                                     ),
                rep(Inf, n_samples - N_DN_trials)
                )
  }
  ...
  RTs <- pmin(DO_RTs, DN_RTs, guess_RTs)
  ...
  accuracy <- numeric(n_samples)
  
  if (N_DO_wins > 0) {
    accuracy[DO_wins] <- rbinom(N_DO_wins, 1, weights["GC_DO"])
  }

  if (N_DN_wins > 0) {
    accuracy[DN_wins] <- rbinom(N_DN_wins, 1, weights["GC_DN"])
  }
  
  if (n_samples < N_DN_wins + N_DO_wins) {
    accuracy[!(DO_wins | DN_wins)] <- rbinom(n_samples - N_DN_wins - N_DO_wins, 1, weights["GC_FD"])
  }
...
DR_obj <- function(theta, data, yoke_detect_means=TRUE, fixed=NULL) {
...
twoLT_DR_GOF_sum <- group_by(twoLT_DR_GOF_subject, model) %>%
  summarise_at(.vars=c("LL", "null_LL", "G2", "df"), "mean")
```
...
ggplot(conditional_quantiles_avg,
  ...
  scale_linetype_discrete(NULL,
                          labels = c("twoLT_yoked" = "Two LT\nTwo Mu's",
                                     "twoLT_unyoked" = "Two LT\nThree Mu's",
                                     "obs_NA" = "Observed")
                          ) +
...
for (s in unique(speeded_RT_choice_quantiles$subject)) {
  ...
  kable(data.frame(Parameter = formatted_names_map[colnames(subject_params)][-1],
                   Yoked = unlist(filter(subject_params, model=="yoked")[-1]),
                   Unyoked = unlist(filter(subject_params, model=="unyoked")[-1])
                   ),
        digits=3,
        row.names=FALSE,
        escape = FALSE) %>%
    ...
    kable(data.frame(Statistic=formatted_names_map[colnames(subject_GOF)[-1]],
                     Yoked = unlist(filter(subject_GOF, model=="yoked")[-1]),
                     Unyoked = unlist(filter(subject_GOF, model=="unyoked")[-1])
                     ),
          digits = 3,
          row.names = FALSE,
          escape = FALSE) %>% 
