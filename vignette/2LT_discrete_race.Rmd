  ...
  for (strength in unique(data$strength)) {
    ...
    if (strength == "L") {
      weights <- c(theta[endsWith(names(theta), strength)],
                   1-theta[c("RO_DN.b", "RO_FD.b", "RO_DO.b")]
                   )
    } else {
      weights <- c(theta[endsWith(names(theta), strength)],
                   theta[c("RO_DN.b", "RO_FD.b", "RO_DO.b")]
                   )
    }
    names(weights)[(length(weights)-2):length(weights)] <- c("GC_DN", "GC_FD", "GC_DO")

    n_samples <- 10000
    p_detect <- sum(theta[paste0(c("DN_", "DO_"), strength)])
...
tidy_params <- bind_rows(mutate(strength_wise_params, speeded_correct = FALSE),
...
predicted_probabilities <- left_join(tidy_params,
...
predicted_quantiles <- Map(DR_quantiles,
                           lapply(twoLT_DR_fits_optimx, function(x) unlist(x[1:11])),
...
conditional_quantiles_subject <- 
  ...
  right_join(predicted_quantiles,
...
## Individual Subjects
```{r subject_plots, cache=TRUE, fig.width=8.5, fig.align="center", results="asis"}
