...
```{r MLE_fit, dependson="cell_counts"}
...
hyperparam_colnames[5] <- "log-\u03BC"
                                
```


```{r MLE_fit_display, dependson=c("formatted_column_names","MLE_fit")}
...
```{r parameter_summaries, cache=TRUE, dependson=c("stan_model","formatted_column_names")}
...
subject_dprimes <- filter(sub_level_params, parameter=="mu_sub", statistic=="median") %>%
  mutate(strength = ifelse(which==1,"Weak","Strong")) %>%
  select(subject, strength, value) %>%
  # unite(col = "strength", strength, statistic) %>%
  spread(strength, value) %>%
  select(subject, Weak, Strong)

sub_sigma <- filter(sub_level_params, parameter=="log_sigma_sub", statistic=="median") %>%
  select(subject, parameter, value) %>%
  spread(parameter, value) %>%
  rename(log_sigma = log_sigma_sub) %>%
  mutate(sigma = exp(log_sigma)) %>%
  select(-log_sigma)

sub_crit <- filter(sub_level_params, parameter=="crit_sub", statistic=="median") %>%
...
hyper <- select(posterior, contains("_pop")) %>%
  ...
  select(mu_pop.1, mu_pop_SD.1, mu_pop.2, mu_pop_SD.2, log_sigma_pop, log_sigma_pop_SD,
         crit_pop.3, crit_pop_SD.3, crit_pop.2, crit_pop_SD.2, crit_pop.1, crit_pop_SD.1)
```

```{r posterior_display, dependson=c('formatted_column_names','parameter_summaries')}
