...
fits <- foreach(p_row = iterators::iter(select(parameters, -id), "row"),
    ...
    list(theta=theta, data=data, fit=fit)
  }


diffusion_fits <- lapply(lapply(fits, `[[`, 'fit'),
                         tibble::rownames_to_column, var="method") %>%
  bind_rows()
GOF <- select(diffusion_fits, fevals, gevals, niter, convcode, value)
bf_parameters <- select(diffusion_fits, !!!names(parameters)[-1])
```
