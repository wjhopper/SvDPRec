...
twoLT_point_predictions <-
  select(.data = twoLT_plus_DR_params_scaled_subject,
         subject, model, DO_L, FD_L, DN_L, DO_W, FD_W, DN_W, DO_S, FD_S, DN_S, b_C, b_N, b_L,
  ...
  summarise(x = list(as.data.frame(
      c(pOld = list(c("25%", "50%", "75%")),
        b = list(setNames(value, parameter)[c("b_C", "b_N", "b_L")]),
...
## Sanity check
# full_join(rename(twoLT_point_predictions, type=model),
#           twoLT_ROC_curve,
#           by =c("subject", "type", "strength")
#           ) %>%
#   mutate(diff <- abs(b.x - b.y)) %>%
#   group_by(subject, type, pOld) %>%
#   filter(diff == min(diff))
  

twoLT_ROC_curve_avg <- group_by(.data = twoLT_ROC_curve, type, strength, b) %>%
  summarise_at(.vars = c("FAR", "HR"), .funs = "mean") %>%
  ungroup()

# twoLT_ROC_curve_avg <-
#   select(.data = twoLT_plus_DR_params_scaled_avg,
#          model, DO_L, FD_L, DO_W, FD_W, DO_S, FD_S
#          ) %>%
...
#          x = list(bias_points)) %>%
#   unnest(x) %>%
#   gather(key="parameter", value="value", -model, -bias_ID) %>%
#   group_by(model, bias_ID) %>%
#   summarise(x = list(as.data.frame(
#       c(pOld = list(c("25%", "50%", "75%")),
#         twoLT(c(calculate_RO_parameters(setNames(value, parameter)[c("b_C","b_N","b_L")]),
...
ggplot(ROC_point_predictions_avg,
  ...
  geom_line(aes(linetype=type, group=interaction(type, strength)),
            data = filter(ROC_point_predictions_avg, type=="obs"),
            size=1.75
            ) +
...
ggplot(ROC_point_predictions,
  ...
  geom_line(aes(linetype=type, group=interaction(strength, type)),
            data = filter(ROC_point_predictions, type=="obs"),
            size=1) +
  geom_line(aes(x=FAR, y=HR, color=strength, linetype=type),
            data = twoLT_ROC_curve,
            inherit.aes = FALSE
            ) +
  facet_wrap(~ subject,
