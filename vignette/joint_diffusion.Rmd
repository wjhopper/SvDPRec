...
optimx_fits <- foreach(sub = subjectwise_datalist,
    ...
    while (n <= 25) {
      ...
      if (n > 2) {
        ...
        parameters[c("crit_old.C", "crit_old.N", "crit_old.L",
                   "crit_new.C", "crit_new.N", "crit_new.L")] <-
          runif(6, c(1, .5, 0, -.5, -1, -1.5), c(1.5, 1, .5, 0, -.5, -1))
      ...
      fit <- optimx(parameters,
                    ...
                    upper = c( 6, 5,     6,  6, 5,    3,    2.99, .75, .75,  4,  4,  4,  4,  4,  4),
...
diffusion_fits <- lapply(optimx_fits, tibble::rownames_to_column, var="method") %>%
  bind_rows() %>%
  group_by(subject) %>%
