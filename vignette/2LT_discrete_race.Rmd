...
parameter_scaling <- function(theta) {
  ...
  theta <- c(theta,
             "DN_L"=NA_real_, "DN_W"=NA_real_, "DN_S"=NA_real_,
             "RO_DN"=NA_real_, "RO_FD"=NA_real_, "RO_DO"=NA_real_
             )
  ...
  theta["RO_DN"] <- abs(pmin(theta["b"], 0))
  theta["RO_FD"] <- pmax(pmin(theta["b"], 1), 0)
  theta["RO_DO"] <-  2 - pmax(theta["b"], 1)
  return(theta)
}
  ...
  for (strength in unique(data$strength)) {
    ...
    if (strength == "L") {
      weights <- c(theta[endsWith(names(theta), strength)],
                   1-theta[c("RO_DN", "RO_FD", "RO_DO")]
                   )
    } else {
      weights <- c(theta[endsWith(names(theta), strength)],
                   theta[c("RO_DN", "RO_FD", "RO_DO")]
                   )
    }
