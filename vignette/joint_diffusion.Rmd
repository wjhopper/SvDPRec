...
diffusion_fits <- foreach(sub = subjectwise_datalist,
                          ...
                          .noexport = c("diffusion_SDT_sim", "zigg_seed", "joint_counts",
    ...
    if (params$use_cpp) {
      sourceCpp(file.path(root_dir, "src", "diffusion.cpp"))
      zigg_seed(as.integer(sub$data$subject[1]))
    } else {
      set.seed(as.integer(sub$data$subject[1]))
    }

    value <- Inf
    n <- 1
    while (value == Inf & n < 100) {
      print(n)
      ...
      if (n > 1) {
        parameters[c("v.L","v.W","v.S")] <- parameters[c("v.L","v.W","v.S")] + runif(3, -1, 1)
      }
      fit <- optimx(parameters,
                    fn = diffusion_obj,
                    method = c("L-BFGS-B", "nlminb"),
                    itnmax = 1000,
                    lower = c(-5, 0.1, -5, -5, 0.1, 0.01, .05, .01, .01, -3, -3, -3, -3, -3, -3),
                    upper = c(5,  4,    5,  5, 4,   3,    .95, .5,  .5,   3,  3,  3, 3,  3,  3),
                    control = list(kkt=FALSE, follow.on=FALSE, maxit=1000),
                    data = sub$data)
      if (fit$value == 1e9) {
        fit$value <- Inf
      }
...
GOF <- bind_rows(diffusion_fits) %>%
  select(subject, niter, convcode, value)

parameters <- bind_rows(diffusion_fits) %>%
  select("subject", "v.L", "sv.L", "v.W", "v.S", "sv.T", "a", "z", "t0", "st0",
         "crit_old.C", "crit_old.N", "crit_old.L", "crit_new.C", "crit_new.N", "crit_new.L")
```
...
```{r predictions}
simulations <- rowwise(conditionwise_params) %>%
  do({
    zigg_seed(1234)
    sim <- diffusion_SDT_sim(N=1e4, a = .$a, z = .$z, t0 = .$t0, st0 = .$st0,
    ...
    quantiles <- group_by(sim, speeded_resp, delayed_resp) %>%
      summarise(quintiles = list(
                data.frame(quantile = c("10%", "30%", "50%", "70%", "90%", "100%"),
    ...
    X <- full_join(proportions, quantiles, by=c("speeded_resp", "delayed_resp"))
    X$subject <- .$subject
    X$strength <- .$strength
    X$pOld <- .$pOld
    X
  ...
  select(subject, strength, pOld, speeded_judgment, delayed_judgment, quantile, RT = value, bin_counts, p)
...
sub_conditional_quantiles <- filter(joint_quantiles, quantile != "100%") %>%
  ...
  select(subject, strength, pOld, speeded_judgment, delayed_judgment, quantile, RT, bin_counts, p)
