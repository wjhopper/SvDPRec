...
```{r start_cluster, dependson=c("discrete_race_exgaussians", "objective_functions")}
...
```{r speeded_quantiles, dependson=c("parameters", "predicted_probabilities")}
...
twoHT_ROC_subject <- twoHT_plus_DR_params_subject %>%
...
twoHT_ROC_avg <- twoHT_plus_DR_params_avg %>%
...
```{r ROC_avg_plot, fig.width=7, fig.height=7, dependson="delayed_acc_predictions"}
ggplot(twoHT_ROC_avg,
  ...
  scale_linetype_discrete(limits = c("unyoked", "yoked"),
...
```{r ROC_subject_plot, dependson="delayed_acc_predictions", fig.height=12, fig.width=9}
ggplot(twoHT_ROC_subject,
       aes(x=FAR, y=HR, color=strength)) +
  geom_line(aes(linetype=model),
            size=.8
            ) +
...
```{r RT_plots, dependson="fit", fig.width=10, results="asis"}
