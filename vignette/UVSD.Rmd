...
UVSD_MLE_params_avg <- summarise_at(UVSD_MLE_params,
                                    .vars = c("d.W", "d.S", "s", "c.C", "c.N", "c.L"),
                                    .funs = "mean")
...
```{r MLE_ROC_avg_plot, fig.width=9}
...
```{r MLE_parameters, dependson=c("MLE_fit")}
...
```{r MLE_ROC_sub_plot, fig.width=9, fig.height=9}
...
```{r Stan_ROC_hyperparmeters_plot}
ggplot(MLE_ROC_preds_avg,
       aes(x=FAR, y=HR, linetype=strength)) +
  geom_line(size=1.75) +
  geom_point(aes(color=factor(pOld)),
...
```{r Stan_ROC_sub_plot, fig.width=9, fig.height=9}
