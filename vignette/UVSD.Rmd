...
fit_SDT <- function(theta, ...) {
  negLL <- SDT_LL(d = theta[c("d.W","d.S")], s = theta[c("s.W","s.S")],
...
```{r fit}
UVSD_fits <- foreach(s = split(delayed_counts_by_sub_bias,
                               delayed_counts_by_sub_bias$subject)
                     ) %do% {
  fit <- optimx(par = c(d=c("W"=1.5, "S"=2.5),
                        s= c("W"=1.15, "S"=1.25),
                        crit = c(1.25, 1, .75)),
              fn = fit_SDT, method = "nlminb",
              itnmax = 3000,
